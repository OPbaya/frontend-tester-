...
import {
  runLoadTest,
  getLoadTestHistory,
  deleteLoadTestHistory,
} from "../services/api";

function LoadTest() {
  // ─────────────────────────────────────────────
  // TAB STATE
  // "run" = Run tab, "history" = History tab
  // ─────────────────────────────────────────────
  ...
  // Convert headers array to object before sending to backend
  ...
  // Color based on verdict for badge
  // function getVerdictBadgeColor(verdict) {
  //   if (verdict === "STABLE")
  ...
  // ─────────────────────────────────────────────
  // FETCH HISTORY
  // Called when user switches to History tab
  // ─────────────────────────────────────────────
  async function fetchHistory() {
    setHistoryLoading(true);
    setHistoryError(null);
    const result = await getLoadTestHistory();
    if (result.success) {
  ...
  // Load history whenever History tab becomes active
  useEffect(() => {
  ...
  async function handleDelete(id) {
    await deleteLoadTestHistory(id);
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="bg-gray-900 border border-gray-800 rounded-xl p-6 space-y-6">
            ...
            <p className="text-gray-500 text-xs">
              {mode === "SEQUENTIAL"
                ...
                : "⚡ Concurrent — sends all requests at the same time. Tests how the API handles load."}
            </p>
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-gray-900 border border-gray-800 rounded-xl p-5 space-y-3">
                  ...
                  <p className="text-gray-500 text-xs">
                    {report.successCount} passed / {report.failureCount} failed
                    out of {report.totalRequests}
                  </p>
                </div>
                ...
                <div className="bg-gray-900 border border-gray-800 rounded-xl p-5 space-y-3">
                  ...
                  <p className="text-gray-500 text-xs">
                    Min: {report.minResponseTime}ms / Max:{" "}
                    {report.maxResponseTime}ms
                  </p>
                </div>

                {/* Status code breakdown with progress bars */}
                <div className="bg-gray-900 border border-gray-800 rounded-xl p-5 space-y-3">
                  <p className="text-gray-400 text-xs uppercase tracking-wide">
                ...
                <div className="bg-gray-900 border border-gray-800 rounded-xl p-5 space-y-3">
                  ...
                  <p className="text-white text-lg font-semibold">
                    {getTrendLabel(report.responseTimeTrend)}
                  </p>
                  <p className="text-gray-500 text-xs">
        ...
        <div className="space-y-4">
          ...
          {!historyLoading && history.length === 0 && (
            <div className="text-center py-16 text-gray-500">
              ...
              <p>No load test history yet.</p>
              <p className="text-sm mt-1">Run a load test first.</p>
            </div>
          )}
            ...
            >
              <div className="flex items-start justify-between gap-4">
                ...
                <div className="space-y-2">
                  ...
                  {/* Date + mode + request count */}
                  <p className="text-gray-500 text-xs">
                    {formatDate(entry.testedAt)} · {entry.mode} ·{" "}
                    {entry.requestCount} requests
                  </p>

                  {/* Verdict + success rate + avg response time */}
                  <div className="flex items-center gap-3">
                    <span
                      className={`text-xs font-semibold px-2 py-0.5 rounded-full ${getVerdictColor(entry.report.verdict)}`}
                    >
                      {entry.report.verdict === "STABLE" && "✅ STABLE"}
                      {entry.report.verdict === "SLOW" && "🐢 SLOW"}
                      {entry.report.verdict === "UNSTABLE" && "⚠️ UNSTABLE"}
                    </span>
                    <span className="text-gray-400 text-xs">
                      {entry.report.successRate.toFixed(1)}% success
                    </span>
                    <span className="text-gray-400 text-xs">
                      {entry.report.avgResponseTime}ms avg
                    </span>
                  </div>
                </div>
      ...
      {/* ══════════════════════════════════════
                FULL REPORT MODAL
                Opens when user clicks a history entry
            ══════════════════════════════════════ */}
      {selectedEntry && (
        <div
        ...
        >
          ...
          >
            ...
            <p className="text-gray-400 text-sm">
              {selectedEntry.mode} · {selectedEntry.requestCount} requests
            </p>

            {/* Verdict */}
            <div
              className={`border rounded-xl p-4 text-center ${getVerdictColor(selectedEntry.report.verdict)}`}
            >
              <p className="text-xl font-bold">
                {selectedEntry.report.verdict === "STABLE" && "✅ STABLE"}
                {selectedEntry.report.verdict === "SLOW" && "🐢 SLOW"}
                {selectedEntry.report.verdict === "UNSTABLE" && "⚠️ UNSTABLE"}
              </p>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-gray-800 rounded-lg p-4">
                ...
                <p className="text-white font-bold text-xl">
                  {selectedEntry.report.successRate.toFixed(1)}%
                </p>
                <p className="text-gray-500 text-xs mt-1">
                  {selectedEntry.report.successCount} passed /{" "}
                  {selectedEntry.report.failureCount} failed
                </p>
              </div>
              <div className="bg-gray-800 rounded-lg p-4">
                <p className="text-gray-400 text-xs uppercase tracking-wide mb-2">
                ...
                <p className="text-white font-bold text-xl">
                  {selectedEntry.report.avgResponseTime}ms avg
                </p>
                <p className="text-gray-500 text-xs mt-1">
                  Min: {selectedEntry.report.minResponseTime}ms / Max:{" "}
                  {selectedEntry.report.maxResponseTime}ms
                </p>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="space-y-2">
                {Object.entries(selectedEntry.report.statusCodeBreakdown).map(
                  ([code, count]) => (
                    <div
                      key={code}
                      className="flex justify-between items-center"
                      ...
                      <div className="flex items-center gap-2">
                        <div className="w-32 bg-gray-700 rounded-full h-1.5">
                          <div
                            ...
                            style={{
                              width: `${(count / selectedEntry.report.totalRequests) * 100}%`,
                            }}
                          />
                        </div>
            ...
            <div>
              ...
              <p className="text-white text-sm">
                {getTrendLabel(selectedEntry.report.responseTimeTrend)}
              </p>
            </div>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y divide-gray-700">
                    {selectedEntry.report.results.map((result, index) => (
                      <tr key={index}>
                        <td className="py-2 pr-4 text-gray-400">
                          {result.requestNumber}
                        </td>
                        ...
                        <td
                          className={`py-2 font-medium ${getResponseTimeColor(result.responseTime)}`}
                        >
                          {result.responseTime}ms
                        </td>
                      </tr>
